 ...
 *
 * Garbage collection:
 *   `AsyncLocalStorage` stores are bound to the active async resource graph.
 *   When the HTTP request completes and no more async frames reference the
 *   chain, the V8 GC reclaims the store. To make this work:
 *     - we ALWAYS pass the session via `als.run(session, callback)` and let the
 *       callback stack die naturally (we never copy the store into module-level
 *       maps or singletons),
 *     - we never hold a reference to the `Session` object beyond the request,
 *     - the auth middleware uses the same `als.run()` once per request, so the
 *       chain ends with the response.
 *
 * Usage from router (set automatically by `authMiddleware()`):
...
export interface Session {
   ...
   * (database seeds, scheduled jobs, migrations) and is only set via
   * `runAsSystem()`.
   */
  actor: string;
 ...
 * jobs). Audit columns will record the literal string `"system"` so it is
 * trivially distinguishable from any user UUID.
 */
