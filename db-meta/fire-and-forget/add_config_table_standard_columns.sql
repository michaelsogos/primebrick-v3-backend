-- Fire-and-forget: Add type/type_config/label_key/description_key/reserved to config_entries.
--
-- The init patch was modified to add:
--   - type, type_config, label_key, description_key, reserved columns to config_entries
--   - group_key column to config_entries
...
--
-- This script:
--   1. Adds the new columns to existing config_entries tables
...
--   9. Creates config_entries_audit table (partitioned, with pg_partman)
...
ALTER TABLE "public"."config_entries" ADD COLUMN IF NOT EXISTS "group_key" varchar(100);

-- 2. Align with Config Table standard: value nullable, key varchar(100), drop description.
ALTER TABLE "public"."config_entries" ALTER COLUMN "value" DROP NOT NULL;
ALTER TABLE "public"."config_entries" ALTER COLUMN "key" TYPE varchar(100);
ALTER TABLE "public"."config_entries" DROP COLUMN IF EXISTS "description";

-- 3. All existing config_entries rows are reserved.
UPDATE "public"."config_entries" SET "reserved" = true WHERE "reserved" = false;

-- 4. Rename casdoor_* keys to idp_* and oidc_issuer_type to idp_type.
UPDATE "public"."config_entries" SET "key" = 'idp_endpoint' WHERE "key" = 'casdoor_endpoint';
UPDATE "public"."config_entries" SET "key" = 'idp_organization' WHERE "key" = 'casdoor_organization';
UPDATE "public"."config_entries" SET "key" = 'idp_client_id' WHERE "key" = 'casdoor_builtin_client_id';
UPDATE "public"."config_entries" SET "key" = 'idp_client_secret' WHERE "key" = 'casdoor_builtin_client_secret';
-- Also handle the previous intermediate rename (idp_builtin_client_* → idp_client_*).
UPDATE "public"."config_entries" SET "key" = 'idp_client_id' WHERE "key" = 'idp_builtin_client_id';
UPDATE "public"."config_entries" SET "key" = 'idp_client_secret' WHERE "key" = 'idp_builtin_client_secret';
UPDATE "public"."config_entries" SET "key" = 'idp_type' WHERE "key" = 'oidc_issuer_type';

-- 5. Delete removed keys.
DELETE FROM "public"."config_entries" WHERE "key" IN ('casdoor_client_id', 'casdoor_admin_username', 'casdoor_admin_role', 'casdoor_admin_password');

-- 6. Seed type/type_config/label_key/description_key for every existing key.
UPDATE "public"."config_entries" SET
  "type" = 'url',
  "type_config" = NULL,
...
UPDATE "public"."config_entries" SET "group_key" = 'idp_parameters' WHERE "key" IN ('idp_endpoint', 'idp_organization', 'auth_mode', 'idp_type');
UPDATE "public"."config_entries" SET "group_key" = 'oidc_parameters' WHERE "key" IN ('oidc_issuer_url', 'oidc_client_id', 'oidc_client_secret', 'auth_roles_path');
UPDATE "public"."config_entries" SET "group_key" = 'security_parameters' WHERE "key" IN ('enable_email_verification_check', 'enable_webauthn', 'passkey_required', 'enable_mfa', 'password_policy', 'mfa_challenge_token_ttl_seconds', 'mfa_challenge_signing_secret');
UPDATE "public"."config_entries" SET "group_key" = 'advanced_features' WHERE "key" IN ('redis_url', 'invitation_expiry_days');
UPDATE "public"."config_entries" SET "group_key" = 'system_settings' WHERE "key" IN ('admin_contact_email', 'frontend_url', 'notification_alert_secret');
UPDATE "public"."config_entries" SET "group_key" = 'idp_parameters' WHERE "key" IN ('idp_client_id', 'idp_client_secret');
...
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"url":{"protocols":["http","https"],"error_label_key":"config.auth.idp_endpoint.errors.invalidUrl"}}}}' WHERE "key" = 'idp_endpoint';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":1,"error_label_key":"config.auth.idp_organization.errors.min"},"max":{"value":100,"error_label_key":"config.auth.idp_organization.errors.max"}}}}' WHERE "key" = 'idp_organization';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"url":{"protocols":["http","https"],"error_label_key":"config.auth.oidc_issuer_url.errors.invalidUrl"}}}}' WHERE "key" = 'oidc_issuer_url';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":6,"error_label_key":"config.auth.oidc_client_id.errors.min"},"max":{"value":100,"error_label_key":"config.auth.oidc_client_id.errors.max"}}}}' WHERE "key" = 'oidc_client_id';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":32,"error_label_key":"config.auth.oidc_client_secret.errors.min"},"max":{"value":256,"error_label_key":"config.auth.oidc_client_secret.errors.max"}}}}' WHERE "key" = 'oidc_client_secret';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":6,"error_label_key":"config.auth.idp_client_id.errors.min"},"max":{"value":100,"error_label_key":"config.auth.idp_client_id.errors.max"}}}}' WHERE "key" = 'idp_client_id';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":32,"error_label_key":"config.auth.idp_client_secret.errors.min"},"max":{"value":256,"error_label_key":"config.auth.idp_client_secret.errors.max"}}}}' WHERE "key" = 'idp_client_secret';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":3,"error_label_key":"config.auth.auth_roles_path.errors.min"},"max":{"value":255,"error_label_key":"config.auth.auth_roles_path.errors.max"},"regex":{"pattern":"^[a-zA-Z0-9._-]+$","error_label_key":"config.auth.auth_roles_path.errors.invalidFormat"}}}}' WHERE "key" = 'auth_roles_path';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":1,"error_label_key":"config.auth.invitation_expiry_days.errors.min"},"max":{"value":90,"error_label_key":"config.auth.invitation_expiry_days.errors.max"}}}}' WHERE "key" = 'invitation_expiry_days';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"email":{"error_label_key":"config.auth.admin_contact_email.errors.invalidEmail"}}}}' WHERE "key" = 'admin_contact_email';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":32,"error_label_key":"config.auth.notification_alert_secret.errors.min"},"max":{"value":256,"error_label_key":"config.auth.notification_alert_secret.errors.max"}}}}' WHERE "key" = 'notification_alert_secret';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"url":{"protocols":["http","https"],"error_label_key":"config.auth.frontend_url.errors.invalidUrl"}}}}' WHERE "key" = 'frontend_url';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"url":{"protocols":["redis","rediss","tcp","http","https"],"error_label_key":"config.auth.redis_url.errors.invalidUrl"}}}}' WHERE "key" = 'redis_url';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":30,"error_label_key":"config.auth.mfa_challenge_token_ttl_seconds.errors.min"},"max":{"value":600,"error_label_key":"config.auth.mfa_challenge_token_ttl_seconds.errors.max"}}}}' WHERE "key" = 'mfa_challenge_token_ttl_seconds';
UPDATE "public"."config_entries" SET "type_config" = '{"validation":{"required":true,"rules":{"min":{"value":32,"error_label_key":"config.auth.mfa_challenge_signing_secret.errors.min"},"max":{"value":256,"error_label_key":"config.auth.mfa_challenge_signing_secret.errors.max"}}}}' WHERE "key" = 'mfa_challenge_signing_secret';

-- 9. Create config_entries_audit table (partitioned, with pg_partman)
...
-- 10. Seed audit trail for existing config_entries rows (INSERT record, version 1).
