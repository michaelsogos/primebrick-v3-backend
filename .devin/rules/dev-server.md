...
3. If the PID belongs to this project's dev server:
   ...
   - Reuse it. The dev server already has the latest code via HMR / `tsx watch`.
...
## Enforcement
- AI agent MUST check port 3001 before running `pnpm run dev`.
- AI agent MUST NOT kill a PID holding port 3001 without explicit user instruction.
- AI agent MUST NOT start a second `pnpm run dev` instance (EADDRINUSE).
